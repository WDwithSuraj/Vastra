...
const getData = (page, limit, sortTo, soryBy, cate) => {
  return axios.get(`https://courageous-tan-skirt.cyclic.app/mensProduct?${cate ? `category=${cate}` : ""
    }&_page=${page}&_limit=${limit}&_sort=${soryBy}&_order=${sortTo}`)
...
export default function Product() {
  ...
  const fetchAndUpdata = async (page, limit, sortTo, soryBy, cate) => {
    try {
      setLoading(true)
      const res = await getData(page, limit, sortTo, soryBy, cate);
  ...
  useEffect(() => {
    fetchAndUpdata(page, limit, sortTo, soryBy, cate)

  }, [page, limit, sortTo, soryBy, cate])
